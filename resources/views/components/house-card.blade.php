...
<article @class(['card card-hover group', 'lg:grid lg:grid-cols-[7fr_5fr]' => $wide])>
    <div @class(['card-media', 'aspect-[4/3] lg:aspect-auto lg:h-full lg:min-h-[440px]' => $wide, 'aspect-[4/3]' => ! $wide])>
        @if ($cover)
            <img src="{{ $cover }}" alt="{{ $coverImage?->alt ?? $house->name }}" loading="lazy" decoding="async" class="card-img img-reveal">
        @else
            <x-image-placeholder :title="$house->name" class="h-full w-full transition-transform duration-700 ease-out group-hover:scale-[1.05]" />
        @endif
    ...
    <div @class(['flex flex-1 flex-col p-6 pb-5', 'lg:justify-center lg:p-10 lg:pb-8' => $wide])>
        ...
        <h3 @class(['mt-3 font-display font-semibold leading-tight text-stone-950 transition-colors group-hover:text-fir-800', 'text-2xl sm:text-3xl lg:text-4xl' => $wide, 'text-2xl' => ! $wide])>
            <a href="{{ route('houses.show', $house) }}" class="card-link focus:outline-none">{{ $house->name }}</a>
        </h3>

        <p @class(['mt-3 text-base leading-7 text-stone-600', 'line-clamp-3 lg:text-lg lg:leading-8' => $wide, 'line-clamp-2' => ! $wide])>{{ $house->short_description }}</p>
