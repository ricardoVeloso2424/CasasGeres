...
@if ($count === 0)
    <x-image-placeholder :title="$title" class="aspect-[16/9] w-full rounded-3xl lg:aspect-[21/9]" />
@elseif ($count === 1)
    <div class="group overflow-hidden rounded-3xl bg-sand-100 ring-1 ring-fir-950/5">
...
@else
    <div class="grid gap-3 lg:grid-cols-[2fr_1fr] lg:gap-4">
        ...
        <div class="grid grid-cols-2 gap-3 lg:gap-4">
            @foreach ($sidePhotos as $photo)
                <div @class(['group overflow-hidden rounded-3xl bg-sand-100 ring-1 ring-fir-950/5', 'col-span-2' => $sidePhotos->count() === 3 && $loop->first])>
